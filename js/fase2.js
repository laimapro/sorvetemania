      ...
      function verificarTodasBolasColetadas() {
        if (score === maxBalls && !todasBolasColetadas) {
          ...
          function avancarEtapa() {
            etapaFase2++; // Atualizar o valor da etapa
            localStorage.setItem('etapa2', etapaFase2); // Armazenar o novo valor na LocalStorage
            etapaElement.textContent = 'Atendendo ao pedido: ' + etapaFase2; // Atualizar o texto exibido na página
          }
          // Exibir o alerta de acordo com o resultado da verificação
          if (pedidoCorreto) {
            if(etapaFase2 >= 1 && etapaFase2 <= 3){
              pontuacaoCerto +=50;
...
function gerapedido3() {
  ...
  var etapaFase2 = localStorage.getItem('etapa2');

  if (etapaFase2 === null) {
    // Se o nível não estiver armazenado, criar a etapa 1 e armazenar na LocalStorage
    localStorage.setItem('etapa2', 1);
    etapaFase2 = 1; // Definir etapa como 1
  }

   // Exibir etapa na página
   var etapaElement = document.createElement('p');
   etapaElement.textContent = 'Atendendo ao pedido: ' + etapaFase2;
