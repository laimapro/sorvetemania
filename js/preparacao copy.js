  ...
  if ('speechSynthesis' in window) {
     ...
     // Extrai os sabores usando expressão regular
      ...
      function updateAudioPosition() {
        // Identifica a bola que está diretamente acima do jogador
        ...
        // Se existir uma bola acima do jogador, atualiza a posição do som com base na posição x da bola
        if (nearestBallAbovePlayer.x !== Infinity) {
            const canvasCenterX = canvas.width / 2;
            const ballCenterX = nearestBallAbovePlayer.x + nearestBallAbovePlayer.width / 2;
