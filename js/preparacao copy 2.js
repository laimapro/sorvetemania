  ...
  if ('speechSynthesis' in window) {
     ...
     // Extrai os sabores usando expressão regular
      ...
      function updateSoundVolume(ball) {
        const distance = calculateDistance(squareX, squareY, ball.x, ball.y);
        const maxDistance = canvas.height + canvas.width; // Valor máximo da distância (diagonal da tela)
        const minDistance = 100; // Distância mínima para o som começar a aumentar
      
        // Calcula o volume do som com base na distância da bola ao jogador
        let volume = 0;
        if (distance < minDistance) {
          volume = 1; // Volume máximo se a bola estiver muito próxima ao jogador
        } else if (distance < maxDistance) {
          volume = 1 - (distance - minDistance) / (maxDistance - minDistance);
        }
      
        // Define o volume para o áudio da bola
        ...
        balls.forEach((ball) => {
          if (ball.stuck) {
            ...
            if (!ball.stuck) {
              ...
              // Calcula a posição X do áudio com base na posição relativa da bola
